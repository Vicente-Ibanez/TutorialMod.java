...
import net.Vicente.tutorialmod.item.modItems.ModSnowball;
...
public class ModSnowGolem extends SnowGolem {

    private static final EntityDataAccessor<Byte> DATA_PUMPKIN_ID = SynchedEntityData.defineId(SnowGolem.class, EntityDataSerializers.BYTE);
    private static final byte PUMPKIN_FLAG = 16;
    private static final float EYE_HEIGHT = 1.7F;

    public ModSnowGolem(EntityType<? extends SnowGolem> p_29902_, Level p_29903_) {
    ...
    public void performRangedAttack(LivingEntity p_29912_, float p_29913_) {
        ModSnowball modsnowball = new ModSnowball(this.level, this);
        double d0 = p_29912_.getEyeY() - (double)1.1F;
        double d1 = p_29912_.getX() - this.getX();
        double d2 = d0 - modsnowball.getY();
        double d3 = p_29912_.getZ() - this.getZ();
        double d4 = Math.sqrt(d1 * d1 + d3 * d3) * (double)0.2F;
        modsnowball.shoot(d1, d2 + d4, d3, 10.6F, 12.0F);
        this.playSound(SoundEvents.SNOW_GOLEM_SHOOT, 1.0F, 0.4F / (this.getRandom().nextFloat() * 0.4F + 0.8F));
        this.level.addFreshEntity(modsnowball);
    }
